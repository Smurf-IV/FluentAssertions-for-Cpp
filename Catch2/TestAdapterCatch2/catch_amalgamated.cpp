        ...
        namespace Detail {
            namespace {
                ...
                resample( URng& rng,
                          ...
                          const std::vector<double>::const_iterator first,
                ...
                static double outlier_variance( Estimate<double> mean,
                                                ...
                                                int n ) {
                    const double sb = stddev.point;
                    double mn = mean.point / n;
                    const double mg_min = mn / 2.;
                    const double sg = (std::min)( mg_min / 4., sb / std::sqrt( n ) );
                    double sg2 = sg * sg;
                    double sb2 = sb * sb;

                    auto c_max = [n, mn, sb2, sg2](const double x ) -> double {
                        const double k = mn - x;
                        const double d = k * k;
                        const double nd = n * d;
                        const double k0 = -n * nd;
                        const double k1 = sb2 - n * sg2 + nd;
                        const double det = k1 * k1 - 4 * sg2 * k0;
                        return static_cast<int>( -2. * k0 /
                                                 ( k1 + std::sqrt( det ) ) );
                    };

                    auto var_out = [n, sb2, sg2](const double c ) {
                        const double nc = n - c;
                ...
                static double
                standard_deviation(const std::vector<double>::const_iterator first,
                                   const std::vector<double>::const_iterator last ) {
                    ...
                    const double variance =
                        std::accumulate( first,
                                         last,
                                         0.,
                                         [m](const double a, const double b ) {
                                             const double diff = b - m;
                                             return a + diff * diff;
                                         } ) /
                        ( last - first );
...
#endif
            bool directCompare(const double lhs, const double rhs ) { return lhs == rhs; }
#if defined( __GNUC__ ) || defined( __clang__ )
#    pragma GCC diagnostic pop
#endif

            double weighted_average_quantile(const int k, const int q, const std::vector<double>::iterator first, const std::vector<double>::iterator last) {
                const auto count = last - first;
                const double idx = (count - 1) * k / static_cast<double>(q);
                const int j = static_cast<int>(idx);
                const double g = idx - j;
                std::nth_element(first, first + j, last);
                const auto xj = first[j];
                if ( directCompare( g, 0 ) ) {
                    return xj;
                }

                const auto xj1 = *std::min_element(first + (j + 1), last);
                return xj + g * (xj1 - xj);
            }
            ...
            classify_outliers( std::vector<double>::const_iterator first,
                ...
                const auto q1 = weighted_average_quantile( 1, 4, copy.begin(), copy.end() );
                const auto q3 = weighted_average_quantile( 3, 4, copy.begin(), copy.end() );
                const auto iqr = q3 - q1;
                const auto los = q1 - ( iqr * 3. );
                const auto lom = q1 - ( iqr * 1.5 );
                const auto him = q3 + ( iqr * 1.5 );
                const auto his = q3 + ( iqr * 3. );
            ...
            double erfc_inv(const double x) {
            ...
            bootstrap_analysis analyse_samples(double confidence_level,
                ...
                const auto n = static_cast<int>(last - first); // seriously, one can't use integral types without hell in C++
                ...
                auto Estimate = [=](double(*f)(std::vector<double>::const_iterator,
                    ...
                    const auto seed = entropy();
                    return std::async(std::launch::async, [=] {
                        std::mt19937 rng(seed);
                        const auto resampled = resample(rng, n_resamples, first, last, f);
                ...
                const double outlier_variance = Detail::outlier_variance(mean_estimate, stddev_estimate, n);
...
bool marginComparison(const double lhs, const double rhs, const double margin) {
...
namespace Catch {
    ...
    void Approx::setMargin(const double newMargin) {
    ...
    void Approx::setEpsilon(const double newEpsilon) {
...
namespace literals {
    Approx operator "" _a(const long double val) {
        return Approx(val);
    }
    Approx operator "" _a(const unsigned long long val) {
        return Approx(val);
    }
...
namespace Catch {
    ...
    AssertionResultData::AssertionResultData(const ResultWas::OfType _resultType, LazyExpression const & _lazyExpression):
...
namespace Catch {
    ...
    Capturer::Capturer( StringRef macroName,
        ...
        auto skipq = [&] (const size_t start, const char quote) {
...
//           case '>':
    ...
    void Capturer::captureValue(const size_t index, std::string const& value ) {
...
namespace Catch {
    ...
    int Session::applyCommandLine(const int argc, char const * const * argv ) {
        if( m_startupExceptions )
            return 1;

        const auto result = m_cli.parse( Clara::Args( argc, argv ) );
    ...
    int Session::applyCommandLine(const int argc, wchar_t const * const * argv ) {

        char **utf8Argv = new char *[ argc ];

        for ( int i = 0; i < argc; ++i ) {
            const int bufSize = WideCharToMultiByte( CP_UTF8, 0, argv[i], -1, nullptr, 0, nullptr, nullptr );
        ...
        const int returnCode = applyCommandLine( argc, utf8Argv );
...
namespace Catch {
    ...
    namespace {
        ...
        bool applies(const TestCaseProperties tcp) {
            static_assert(static_cast<TCP_underlying_type>(TestCaseProperties::None) == 0,
                          "TestCaseProperties::None must be equal to 0");
            return tcp != TestCaseProperties::None;
        }

        TestCaseProperties parseSpecialTag(const StringRef tag ) {
            if( !tag.empty() && tag[0] == '.' )
        ...
        bool isReservedTag(const StringRef tag ) {
            return parseSpecialTag( tag ) == TestCaseProperties::None
                && tag.size() > 0
                && !std::isalnum( static_cast<unsigned char>(tag[0]) );
        }
        void enforceNotReservedTag(const StringRef tag, SourceLineInfo const& _lineInfo ) {
        ...
        StringRef extractFilenamePart(const StringRef filename) {
        ...
        size_t sizeOfExtraTags(const StringRef filepath) {
            // [.] is 3, [#] is another 3
    ...
    bool operator<(  Tag const& lhs, Tag const& rhs ) {
        constexpr Detail::CaseInsensitiveLess cmp;
        return cmp( lhs.original, rhs.original );
    }
    bool operator==( Tag const& lhs, Tag const& rhs ) {
        constexpr Detail::CaseInsensitiveEqualTo cmp;
        return cmp( lhs.original, rhs.original );
    }
    ...
    TestCaseInfo::TestCaseInfo(const StringRef _className,
        ...
        lineInfo( _lineInfo )
    {
        const StringRef originalTags = _nameAndTags.tags;
        // We need to reserve enough space to store all of the tags
        // (including optional hidden tag and filename tag)
        const auto requiredSize = originalTags.size() + sizeOfExtraTags(_lineInfo.file);
    ...
    void TestCaseInfo::internalAppendTag(const StringRef tagStr) {
        backingTags += '[';
...
namespace Detail {
    ...
    constexpr int hexThreshold = 255;
        ...
        struct Endianness {
            ...
            static Arch which() {
                ...
                const auto value = *reinterpret_cast<char*>(&one);
                return value ? Little : Big;
            }
        };

        template<typename T>
        std::string fpToString(T value, const int precision) {
    ...
    std::string convertIntoString(const StringRef string, const bool escape_invisibles) {
        std::string ret;
    ...
    std::string convertIntoString(const StringRef string) {
        return convertIntoString(string, getCurrentContext().getConfig()->showInvisibles());
    }

    std::string rawMemoryToString( const void *object, const std::size_t size ) {
...
std::string StringMaker<std::string_view>::convert(const std::string_view str) {
...
# ifdef CATCH_CONFIG_CPP17_STRING_VIEW
std::string StringMaker<std::wstring_view>::convert(const std::wstring_view str) {
...
#include <cstddef>
std::string StringMaker<std::byte>::convert(const std::byte value) {
    return ::Catch::Detail::stringify(std::to_integer<unsigned long long>(value));
}
#endif // defined(CATCH_CONFIG_CPP17_BYTE)

std::string StringMaker<int>::convert(const int value) {
    return ::Catch::Detail::stringify(static_cast<long long>(value));
}
std::string StringMaker<long>::convert(const long value) {
    return ::Catch::Detail::stringify(static_cast<long long>(value));
}
std::string StringMaker<long long>::convert(const long long value) {
    ReusableStringStream rss;
    rss << value;
...
std::string StringMaker<unsigned int>::convert(const unsigned int value) {
    return ::Catch::Detail::stringify(static_cast<unsigned long long>(value));
}
std::string StringMaker<unsigned long>::convert(const unsigned long value) {
    return ::Catch::Detail::stringify(static_cast<unsigned long long>(value));
}
std::string StringMaker<unsigned long long>::convert(const unsigned long long value) {
    ReusableStringStream rss;
    rss << value;
    if (value > Detail::hexThreshold) {
        rss << " (0x" << std::hex << value << ')';
...
std::string StringMaker<signed char>::convert(const signed char value) {
    if (value == '\r') {
        return "'\\r'";
    } else if (value == '\f') {
        return "'\\f'";
...
std::string StringMaker<char>::convert(const char c) {
    return ::Catch::Detail::stringify(static_cast<signed char>(c));
}
std::string StringMaker<unsigned char>::convert(const unsigned char c) {
    return ::Catch::Detail::stringify(static_cast<char>(c));
}

int StringMaker<float>::precision = 5;

std::string StringMaker<float>::convert(const float value) {
    return Detail::fpToString(value, precision) + 'f';
}

int StringMaker<double>::precision = 10;

std::string StringMaker<double>::convert(const double value) {
...
namespace Catch {
    ...
    Version::Version
        (const unsigned int _majorVersion,
         ...
         const unsigned int _patchNumber,
         char const * const _branchName,
         const unsigned int _buildNumber )
...
namespace Detail {
    ...
    IGeneratorTracker* acquireGeneratorTracker(const StringRef generatorName, SourceLineInfo const& lineInfo ) {
        return getResultCapture().acquireGeneratorTracker( generatorName, lineInfo );
    }

    IGeneratorTracker* createGeneratorTracker(const StringRef generatorName,
...
namespace Catch {
    ...
    SectionStats::SectionStats(  SectionInfo&& _sectionInfo,
                                 ...
                                 const double _durationInSeconds,
                                 const bool _missingAssertions )
...
namespace Catch {
    ...
    AssertionHandler::AssertionHandler
        (const StringRef macroName,
         SourceLineInfo const& lineInfo,
         const StringRef capturedExpression,
         const ResultDisposition::Flags resultDisposition )
    ...
    void AssertionHandler::handleMessage(const ResultWas::OfType resultType, const StringRef message) {
...
namespace Catch {
    namespace Detail {
        ...
        bool CaseInsensitiveLess::operator()(const StringRef lhs,
                                             const StringRef rhs ) const {
        ...
        bool
        CaseInsensitiveEqualTo::operator()(const StringRef lhs,
                                           const StringRef rhs ) const {
            return std::equal(
                lhs.begin(), lhs.end(),
                rhs.begin(), rhs.end(),
...
namespace Catch {
    namespace Clara {
        namespace Detail {
            ...
            TokenStream::TokenStream(const Iterator it_, const Iterator itEnd_ ):
                it( it_ ), itEnd( itEnd_ ) {
                loadBuffer();
            }
            ...
            ParserResult convertInto( std::string const& source,
                                      bool& target ) {
                const std::string srcLC = toLower( source );
            ...
            ParseState::ParseState(const ParseResultType type,
                                   TokenStream const& remainingTokens ):
                m_type( type ), m_remainingTokens( remainingTokens ) {}

            ParserResult BoundFlagRef::setFlag(const bool flag ) {
                m_ref = flag;
                return ParserResult::ok( ParseResultType::Matched );
            }
        ...
        Detail::InternalParseResult Arg::parse(std::string const&,
            ...
            const auto result = valueRef->setValue(remainingTokens->token);
            if (!result)
                    ...
                    if (m_ref->isFlag()) {
                        const auto flagRef =
                            static_cast<Detail::BoundFlagRefBase*>(
                                m_ref.get());
                        const auto result = flagRef->setFlag(true);
                        if (!result)
                            return Detail::InternalParseResult(result);
                        if (result.value() ==
        ...
        ParserResult ExeName::set(std::string const& newName) const
        {
        ...
        void Parser::writeToStream( std::ostream& os ) const {
            ...
            const auto rows = getHelpColumns();
            constexpr size_t consoleWidth = CATCH_CONFIG_CONSOLE_WIDTH;
            size_t optWidth = 0;
        ...
        Args::Args(const int argc, char const* const* argv) :
            m_exeName(argv[0]), m_args(argv + 1, argv + argc) {}

        Args::Args(const std::initializer_list<std::string> args) :
...
namespace Catch {
    ...
    Clara::Parser makeCommandLineParser( ConfigData& config ) {
        ...
        auto const setWaitForKeypress = [&]( std::string const& keypress ) {
            const auto keypressLc = toLower( keypress );
                if (keypressLc == "never")
        ...
        auto const setVerbosity = [&]( std::string const& verbosity ) {
            const auto lcVerbosity = toLower( verbosity );
        ...
        auto const setReporter = [&]( std::string const& userReporterSpec ) {
            ...
            const auto result = factories.find( reporterSpec.name() );
        ...
        auto cli
            ...
            | Opt( [&](const int x ){ config.abortAfter = x; }, "no. failures" )
                ["-x"]["--abortx"]
                ( "abort after x failures" )
            | Opt( accept_many, setWarning, "warning name" )
                ["-w"]["--warn"]
                ( "enable warnings" )
            | Opt( [&](const bool flag ) { config.showDurations = flag ? ShowDurations::Always : ShowDurations::Never; }, "yes|no" )
                ["-d"]["--durations"]
                ( "show test durations" )
...
namespace Catch {
    ...
    ColourImpl::ColourGuard ColourImpl::guardColour(const Colour::Code colourCode ) {
    ...
    ColourImpl::ColourGuard::ColourGuard(const Colour::Code code,
...
namespace {
    ...
    private:
        void use(const Colour::Code _colourCode ) const override {
            switch( _colourCode ) {
        ...
        void setTextAttribute(const WORD _textAttribute ) const {
...
namespace Catch {
    ...
    bool isColourImplAvailable(const ColourMode colourSelection ) {
...
namespace Catch {
    ...
    void formatReconstructedExpression( std::ostream &os, std::string const& lhs, const StringRef op, std::string const& rhs ) {
...
namespace Catch {
    ...
    namespace Detail {
        ...
        namespace {
            ...
            StringRef extractInstanceName(const StringRef enumInstance) {
        ...
        std::vector<StringRef> parseEnums(const StringRef enums ) {
            const auto enumValues = splitStringRef( enums, ',' );
        ...
        StringRef EnumInfo::lookup(const int value ) const {
        ...
        Catch::Detail::unique_ptr<EnumInfo> makeEnumInfo(const StringRef enumName, const StringRef allValueNames, std::vector<int> const& values ) {
        ...
        EnumInfo const& EnumValuesRegistry::registerEnum(const StringRef enumName, const StringRef allValueNames, std::vector<int> const& values ) {
...
namespace Catch {
    ...
    static LONG CALLBACK topLevelExceptionFilter(const PEXCEPTION_POINTERS ExceptionInfo) {
...
namespace Catch {
    namespace Detail {
        ...
        uint32_t convertToBits(const float f) {
        ...
        uint64_t convertToBits(const double d) {
...
namespace Catch {
    namespace {
        ...
        void listTests(IEventListener& reporter, IConfig const& config) {
            ...
            const auto matchedTestCases = filterTests(getAllTestCasesSorted(config), testSpec, config);
            reporter.listTests(matchedTestCases);
        }

        void listTags(IEventListener& reporter, IConfig const& config) {
            auto const& testSpec = config.testSpec();
            const std::vector<TestCaseHandle> matchedTestCases = filterTests(getAllTestCasesSorted(config), testSpec, config);
    ...
    void TagInfo::add(const StringRef spelling ) {
        ++count;
...
namespace Catch {
    ...
    MessageInfo::MessageInfo(const StringRef _macroName,
                             SourceLineInfo const& _lineInfo,
                             const ResultWas::OfType _type )
...
namespace Catch {
    ...
    Optional<unsigned int> parseUInt(std::string const& input, const int base) {
...
#pragma warning(disable:4146) // we negate uint32 during the rotate
        ...
        uint32_t rotate_right(const uint32_t val, uint32_t count) {
...
#pragma warning(pop)
    ...
    SimplePcg32::SimplePcg32(const result_type seed_) {
        seed(seed_);
    }


    void SimplePcg32::seed(const result_type seed_) {
        m_state = 0;
        (*this)();
        m_state += seed_;
        (*this)();
    ...
    void SimplePcg32::discard(const uint64_t skip) {
...
namespace Catch {
    ...
    std::uint32_t generateRandomSeed(const GenerateFrom from ) {
        switch ( from ) {
...
namespace Catch {
    ...
    ReporterRegistry::create( std::string const& name,
        ...
        const auto it = m_impl->factories.find( name );
    ...
    void ReporterRegistry::registerReporter( std::string const& name,
        ...
        CATCH_ENFORCE( name.find( "::" ) == name.npos,
                       "'::' is not allowed in reporter name: '" + name +
                           '\'' );
        const auto ret = m_impl->factories.emplace( name, CATCH_MOVE( factory ) );
...
namespace Catch {
    ...
    namespace {
        ...
        kvPair splitKVPair(const StringRef kvString) {
            const auto splitPos = static_cast<size_t>(
    ...
    namespace Detail {
        std::vector<std::string> splitReporterSpec( StringRef reporterSpec ) {
            ...
            auto findNextSeparator = [&reporterSpec](const size_t startPos ) {
                static_assert(
                    separatorSize == 2,
        ...
        Optional<ColourMode> stringToColourMode(const StringRef colourMode ) {
    ...
    Optional<ReporterSpec> parseReporterSpec(const StringRef reporterSpec ) {
        ...
        for ( size_t i = 1; i < parts.size(); ++i ) {
            ...
            if ( key.empty() || value.empty() ) {
                ...
                const auto ret = kvPairs.emplace( std::string(kv.key), std::string(kv.value) );
                if ( !ret.second ) {
...
namespace Catch {
    ...
    bool isOk(const ResultWas::OfType resultType ) {
        return ( resultType & ResultWas::FailureBit ) == 0;
    }
    bool isJustInfo(const int flags ) {
        return flags == ResultWas::Info;
    }

    ResultDisposition::Flags operator | (const ResultDisposition::Flags lhs, const ResultDisposition::Flags rhs ) {
        return static_cast<ResultDisposition::Flags>( static_cast<int>( lhs ) | static_cast<int>( rhs ) );
    }

    bool shouldContinueOnFailure(const int flags )    { return ( flags & ResultDisposition::ContinueOnFailure ) != 0; }
    bool shouldSuppressFailure(const int flags )      { return ( flags & ResultDisposition::SuppressFail ) != 0; }
...
namespace Catch {
            ...
            struct GeneratorTracker final : TestCaseTracking::TrackerBase,
                ...
                acquire( TrackerContext& ctx,
                    ...
                    if ( currentTracker.nameAndLocation() == nameAndLocation ) {
                        const auto thisTracker = currentTracker.parent()->findChild(
                            nameAndLocation );
                        assert( thisTracker );
                ...
                void close() override {
                    ...
                    const bool should_wait_for_child = [&]() {
                        ...
                        // wait for them
                        const ITracker* parent = m_parent;
    ...
    bool RunContext::sectionStarted(const StringRef sectionName,
        ...
        const SectionInfo sectionInfo( sectionLineInfo, static_cast<std::string>(sectionName) );
    ...
    IGeneratorTracker*
    RunContext::acquireGeneratorTracker(const StringRef generatorName,
                                        SourceLineInfo const& lineInfo ) {
    ...
    void RunContext::benchmarkPreparing(const StringRef name ) {
    ...
    void RunContext::benchmarkFailed(const StringRef error ) {
    ...
    void RunContext::runCurrentTest(std::string & redirectedCout, std::string & redirectedCerr) {
        ...
        const SectionStats testCaseSectionStats(CATCH_MOVE(testCaseSection), assertions, duration, missingAssertions);
        m_reporter->sectionEnded(testCaseSectionStats);
    }
    ...
    void RunContext::invokeActiveTestCase() {
        ...
        const FatalConditionHandlerGuard _(&m_fatalConditionhandler);
    ...
    void RunContext::reportExpr(
            AssertionInfo const &info,
            const ResultWas::OfType resultType,
            ITransientExpression const *expr,
            const bool negated ) {
...
namespace Catch {
    ...
    bool startsWith(const StringRef s, const char prefix ) {
        return !s.empty() && s[0] == prefix;
    }
    bool endsWith( std::string const& s, std::string const& suffix ) {
        return s.size() >= suffix.size() && std::equal(suffix.rbegin(), suffix.rend(), s.rbegin());
    }
    bool endsWith( std::string const& s, const char suffix ) {
    ...
    StringRef trim(const StringRef ref) {
    ...
    std::vector<StringRef> splitStringRef(const StringRef str, const char delimiter ) {
...
namespace Catch {
    ...
    bool StringRef::operator<(const StringRef rhs) const noexcept {
    ...
    int StringRef::compare(const StringRef rhs ) const {
...
namespace Catch {
    ...
    TagAlias const* TagAliasRegistry::find( std::string const& alias ) const {
        const auto it = m_registry.find( alias );
    ...
    std::string TagAliasRegistry::expandAliases( std::string const& unexpandedTestSpec ) const {
        ...
        for( auto const& registryKvp : m_registry ) {
            const std::size_t pos = expandedTestSpec.find( registryKvp.first );
...
namespace Catch {
    TestCaseInfoHasher::TestCaseInfoHasher(const hash_t seed ): m_seed( seed ) {}
...
namespace Catch {
    ...
    std::vector<TestCaseHandle> sortTests( IConfig const& config, std::vector<TestCaseHandle> const& unsortedTestCases ) {
        ...
        case TestRunOrder::Randomized: {
            ...
            const TestCaseInfoHasher h{ config.rngSeed() };
...
namespace TestCaseTracking {
        ...
        m_trimmed_name(trim(StringRef(ITracker::nameAndLocation().name)))
    {
        if( parent ) {
            ...
            const SectionTracker& parentSection = static_cast<SectionTracker&>( *parent );
...
namespace Catch {
    ...
    namespace {
        static StringRef extractClassName(const StringRef classOrMethodName ) {
            ...
            const auto reverseStart = std::make_reverse_iterator( methodName.end() );
            const auto reverseEnd = std::make_reverse_iterator( methodName.begin() );
            ...
            const auto lastColons = std::find( reverseStart, reverseEnd, ':' ) + 1;
            const auto secondLastColons =
        ...
        public:
            TestInvokerAsFunction(const TestType testAsFunction ) noexcept:
    ...
    AutoReg::AutoReg( Detail::unique_ptr<ITestInvoker> invoker, SourceLineInfo const& lineInfo, const StringRef classOrMethod, NameAndTags const& nameAndTags ) noexcept {
        CATCH_TRY {
...
namespace Catch {
    ...
    bool TestSpecParser::visitChar(const char c ) {
    ...
    bool TestSpecParser::processNoneChar(const char c ) {
        switch( c ) {
        case ' ':
            return true;
        case '~':
    ...
    void TestSpecParser::processNameChar(const char c ) {
        if( c == '[' ) {
    ...
    bool TestSpecParser::processOtherChar(const char c ) {
        if( !isControlChar( c ) )
            return false;
        m_substring += c;
        endMode();
    ...
    void TestSpecParser::startNewMode(const Mode mode ) {
        m_mode = mode;
    }
    ...
    bool TestSpecParser::isControlChar(const char c ) const {
        switch( m_mode ) {
            default:
                return false;
            case None:
...
namespace {
    ...
    bool isBreakableBefore(const char c ) {
        static constexpr char chars[] = "[({<|";
        return std::memchr( chars, c, sizeof( chars ) - 1 ) != nullptr;
    }

    bool isBreakableAfter(const char c ) {
        static constexpr char chars[] = "])}>.,:;*+-=&/\\";
        return std::memchr( chars, c, sizeof( chars ) - 1 ) != nullptr;
    }

    bool isBoundary( std::string const& line, const size_t at ) {
        assert( at > 0 );
        assert( at <= line.size() );
...
namespace Catch {
    namespace TextFlow {
        ...
        std::string
        Column::const_iterator::addIndentAndSuffix(const size_t position,
        ...
        Column Spacer(const size_t spaceWidth ) {
            Column ret{ "" };
...
namespace Catch {
    ...
    WildcardPattern::WildcardPattern( std::string const& pattern,
                                      const CaseSensitive caseSensitivity )
...
namespace {
    ...
    bool shouldNewline(const XmlFormatting fmt) {
        return !!(static_cast<std::underlying_type_t<XmlFormatting>>(fmt & XmlFormatting::Newline));
    }

    bool shouldIndent(const XmlFormatting fmt) {
    ...
    XmlEncode::XmlEncode(const StringRef str, const ForWhat forWhat )
    :   m_str( str ),
        m_forWhat( forWhat )
    {}
    ...
    void XmlEncode::encodeTo( std::ostream& os ) const {
        ...
        for( std::size_t idx = 0; idx < m_str.size(); ++ idx ) {
            const unsigned char c = static_cast<unsigned char>(m_str[idx]);
            switch (c) {
            case '<':   os << "&lt;"; break;
            case '&':   os << "&amp;"; break;
            ...
            default:
                ...
                for (std::size_t n = 1; n < encBytes; ++n) {
                    const unsigned char nc = static_cast<unsigned char>(m_str[idx + n]);
                    valid &= ((nc & 0xC0) == 0x80);
    ...
    XmlWriter::ScopedElement::ScopedElement( XmlWriter* writer, const XmlFormatting fmt )
    :   m_writer( writer ),
        m_fmt(fmt)
    {}
    ...
    XmlWriter::ScopedElement::writeText(const StringRef text, const XmlFormatting fmt ) {
        m_writer->writeText( text, fmt );
        return *this;
    }

    XmlWriter::ScopedElement&
    XmlWriter::ScopedElement::writeAttribute(const StringRef name,
                                             const StringRef attribute ) {
    ...
    XmlWriter& XmlWriter::startElement( std::string const& name, const XmlFormatting fmt ) {
        ensureTagClosed();
        newlineIfNecessary();
        if (shouldIndent(fmt)) {
            m_os << m_indent;
    ...
    XmlWriter::ScopedElement XmlWriter::scopedElement( std::string const& name, const XmlFormatting fmt ) {
        ScopedElement scoped( this, fmt );
        startElement( name, fmt );
        return scoped;
    }

    XmlWriter& XmlWriter::endElement(const XmlFormatting fmt) {
    ...
    XmlWriter& XmlWriter::writeAttribute(const StringRef name,
                                         const StringRef attribute ) {
    ...
    XmlWriter& XmlWriter::writeAttribute(const StringRef name, const bool attribute ) {
    ...
    XmlWriter& XmlWriter::writeText(const StringRef text, const XmlFormatting fmt ) {
    ...
    XmlWriter& XmlWriter::writeComment(const StringRef text, const XmlFormatting fmt ) {
        ensureTagClosed();
        if (shouldIndent(fmt)) {
            m_os << m_indent;
        }
    ...
    void XmlWriter::writeStylesheetRef(const StringRef url ) {
    ...
    void XmlWriter::applyFormatting(const XmlFormatting fmt) {
...
namespace Matchers {
    ...
    HasSizeMatcher SizeIs(const std::size_t sz) {
...
template <typename FP>
FP step(FP start, FP direction, const uint64_t steps) {
...
namespace Detail {
    ...
    WithinAbsMatcher::WithinAbsMatcher(const double target, const double margin)
    ...
    WithinUlpsMatcher::WithinUlpsMatcher(const double target, const uint64_t ulps, const Detail::FloatingPointKind baseType)
...
#pragma clang diagnostic pop
    ...
    WithinRelMatcher::WithinRelMatcher(const double target, const double epsilon):
        m_target(target),
        m_epsilon(epsilon){
...
WithinUlpsMatcher WithinULP(const double target, const uint64_t maxUlpDiff) {
    return WithinUlpsMatcher(target, maxUlpDiff, Detail::FloatingPointKind::Double);
}

WithinUlpsMatcher WithinULP(const float target, const uint64_t maxUlpDiff) {
    return WithinUlpsMatcher(target, maxUlpDiff, Detail::FloatingPointKind::Float);
}

WithinAbsMatcher WithinAbs(const double target, const double margin) {
    return WithinAbsMatcher(target, margin);
}

WithinRelMatcher WithinRel(const double target, const double eps) {
    return WithinRelMatcher(target, eps);
}

WithinRelMatcher WithinRel(const double target) {
    return WithinRelMatcher(target, std::numeric_limits<double>::epsilon() * 100);
}

WithinRelMatcher WithinRel(const float target, const float eps) {
    return WithinRelMatcher(target, eps);
}

WithinRelMatcher WithinRel(const float target) {
...
namespace Matchers {
    ...
    CasedString::CasedString( std::string const& str, const CaseSensitive caseSensitivity )
    :   m_caseSensitivity( caseSensitivity ),
    ...
    StringMatcherBase::StringMatcherBase(const StringRef operation, CasedString const& comparator )
    ...
    RegexMatcher::RegexMatcher(std::string regex, const CaseSensitive caseSensitivity): m_regex(CATCH_MOVE(regex)), m_caseSensitivity(caseSensitivity) {}
    ...
    StringEqualsMatcher Equals( std::string const& str, const CaseSensitive caseSensitivity ) {
        return StringEqualsMatcher( CasedString( str, caseSensitivity) );
    }
    StringContainsMatcher ContainsSubstring( std::string const& str, const CaseSensitive caseSensitivity ) {
        return StringContainsMatcher( CasedString( str, caseSensitivity) );
    }
    EndsWithMatcher EndsWith( std::string const& str, const CaseSensitive caseSensitivity ) {
        return EndsWithMatcher( CasedString( str, caseSensitivity) );
    }
    StartsWithMatcher StartsWith( std::string const& str, const CaseSensitive caseSensitivity ) {
        return StartsWithMatcher( CasedString( str, caseSensitivity) );
    }

    RegexMatcher Matches(std::string const& regex, const CaseSensitive caseSensitivity) {
...
namespace Matchers {
    ...
    namespace Detail {
        ...
        std::string describe_multi_matcher(const StringRef combine, std::string const* descriptions_begin, std::string const* descriptions_end) {
            std::string description;
...
namespace Catch {
    ...
    void handleExceptionMatchExpr( AssertionHandler& handler, StringMatcher const& matcher ) {
        ...
        const MatchExpr<std::string, StringMatcher const&> expr( CATCH_MOVE(exceptionMessage), matcher );
...
public:
    ...
    AssertionPrinter(std::ostream& _stream, AssertionStats const& _stats, const bool _printInfoMessages, ColourImpl* colourImpl_)
        : stream(_stream)
...
private:
    ...
    void printResultType(const Colour::Code colour, const StringRef passOrFail) const {
        if (!passOrFail.empty()) {
    ...
    void printRemainingMessages(const Colour::Code colour = compactDimColour) {
        if (itMessage == messages.end())
            return;
...
private:
    ...
    ColourImpl* colourImpl;
        ...
        void CompactReporter::noMatchingTestCases(const StringRef unmatchedSpec ) {
        ...
        void CompactReporter::sectionEnded(SectionStats const& _sectionStats) {
            const double dur = _sectionStats.durationInSeconds;
...
public:
    ...
    ConsoleAssertionPrinter(std::ostream& _stream, AssertionStats const& _stats, ColourImpl* colourImpl_, const bool _printInfoMessages)
        : stream(_stream),
        stats(_stats),
        result(_stats.assertionResult),
        colour(Colour::None),
...
std::size_t makeRatio(const std::uint64_t number, const std::uint64_t total ) {
...
public:
    explicit Duration(const double inNanoseconds, const Unit units = Unit::Auto)
...
public:
    ...
    void open() {
        if (!m_isOpen) {
            ...
            *this << RowBreak();
            ...
            const auto spacer = TextFlow::Spacer(2);
    ...
    friend TablePrinter& operator << (TablePrinter& tp, ColumnBreak) {
        ...
        const auto padding = (strSize + 1 < colInfo.width)
...
void ConsoleReporter::noMatchingTestCases(const StringRef unmatchedSpec ) {
    m_stream << "No test cases matched '" << unmatchedSpec << "'\n";
}

void ConsoleReporter::reportInvalidTestSpec(const StringRef arg ) {
...
void ConsoleReporter::assertionEnded(AssertionStats const& _assertionStats) {
    ...
    const ConsoleAssertionPrinter printer(m_stream, _assertionStats, m_colour.get(), includeResults);
    printer.print();
    m_stream << '\n' << std::flush;
}
...
void ConsoleReporter::benchmarkPreparing(const StringRef name ) {
	lazyPrintWithoutClosingBenchmarkTable();

    const auto nameCol = TextFlow::Column( static_cast<std::string>( name ) )
...
void ConsoleReporter::benchmarkFailed(const StringRef error ) {
...
void ConsoleReporter::printTestCaseAndSectionHeader() {
    ...
    const SourceLineInfo lineInfo = m_sectionStack.back().lineInfo;
...
void ConsoleReporter::printHeaderString(std::string const& _string, const std::size_t indent) {
...
namespace Catch {
    ...
    CumulativeReporterBase::sectionStarting( SectionInfo const& sectionInfo ) {
        ...
        if ( m_sectionStack.empty() ) {
            ...
            const auto it = std::find_if( parentNode.childSections.begin(),
...
namespace Catch {
    ...
    bool shouldShowDuration( IConfig const& config, const double duration ) {
    ...
    std::ostream& operator<<( std::ostream& out, const lineOfChars value ) {
    ...
    void defaultListTests(std::ostream& out, ColourImpl* streamColour, std::vector<TestCaseHandle> const& tests, const bool isFiltered, const Verbosity verbosity) {
    ...
    namespace {
        class SummaryColumn {
        public:
            SummaryColumn( std::string suffix, const Colour::Code colour ):
                m_suffix( CATCH_MOVE( suffix ) ), m_colour( colour ) {}

            SummaryColumn&& addRow(const std::uint64_t count ) && {
            ...
            std::string const& getRow(const std::size_t index ) const {
                return m_rows[index];
            }
...
namespace Catch {
    ...
    void JunitReporter::writeRun( TestRunNode const& testRunNode, const double suiteTime ) {
...
namespace Catch {
    ...
    void MultiReporter::noMatchingTestCases(const StringRef unmatchedSpec ) {
        for ( auto& reporterish : m_reporterLikes ) {
            reporterish->noMatchingTestCases( unmatchedSpec );
        }
    }

    void MultiReporter::fatalErrorEncountered(const StringRef error ) {
        for ( auto& reporterish : m_reporterLikes ) {
            reporterish->fatalErrorEncountered( error );
        }
    }

    void MultiReporter::reportInvalidTestSpec(const StringRef arg ) {
        for ( auto& reporterish : m_reporterLikes ) {
            reporterish->reportInvalidTestSpec( arg );
        }
    }

    void MultiReporter::benchmarkPreparing(const StringRef name ) {
        for (auto& reporterish : m_reporterLikes) {
    ...
    void MultiReporter::benchmarkFailed(const StringRef error ) {
        for (auto& reporterish : m_reporterLikes) {
...
namespace Catch {
    ...
    void SonarQubeReporter::writeTestFile(const StringRef filename, std::vector<TestCaseNode const*> const& testCaseNodes) {
    ...
    void SonarQubeReporter::writeSection(std::string const& rootName, SectionNode const& sectionNode, const bool okToFail) {
        std::string name = trim(sectionNode.stats.sectionInfo.name);
        if (!rootName.empty())
            name = rootName + '/' + name;
    ...
    void SonarQubeReporter::writeAssertions(SectionNode const& sectionNode, const bool okToFail) {
        for (auto const& assertionOrBenchmark : sectionNode.assertionsAndBenchmarks) {
            if (assertionOrBenchmark.isAssertion()) {
    ...
    void SonarQubeReporter::writeAssertion(AssertionStats const& stats, const bool okToFail) {
        AssertionResult const& result = stats.assertionResult;
        if ( !result.isOk() ||
             result.getResultType() == ResultWas::ExplicitSkip ) {
            std::string elementName;
...
namespace Catch {
    ...
    namespace {
        ...
        public:
            ...
            TapAssertionPrinter(std::ostream& _stream, AssertionStats const& _stats, const std::size_t _counter, ColourImpl* colour_)
                : stream(_stream)
                , result(_stats.assertionResult)
                , messages(_stats.infoMessages)
                , itMessage(_stats.infoMessages.begin())
        ...
        private:
            void printResultType(const StringRef passOrFail) const {
                if (!passOrFail.empty()) {
                    stream << passOrFail << ' ' << counter << " -";
                }
            }

            void printIssue(const StringRef issue) const {
                stream << ' ' << issue;
            }
            ...
            void printRemainingMessages(const Colour::Code colour = tapDimColour) {
                if (itMessage == messages.end()) {
                    return;
                }
    ...
    void TAPReporter::noMatchingTestCases(const StringRef unmatchedSpec ) {
        m_stream << "# No test cases matched '" << unmatchedSpec << "'\n";
    }
...
namespace Catch {
    ...
    namespace {
        ...
        // subsequent lines
        void printHeaderString(std::ostream& os, std::string const& _string, const std::size_t indent = 0) {
        ...
        std::string escape(const StringRef str) {
    ...
    void TeamCityReporter::printSectionHeader(std::ostream& os) {
        ...
        const SourceLineInfo lineInfo = m_sectionStack.front().lineInfo;
...
namespace Catch {
    ...
    void XmlReporter::testRunStarting( TestRunInfo const& testInfo ) {
        ...
        const std::string stylesheetRef = getStylesheetRef();
    ...
    void XmlReporter::benchmarkPreparing(const StringRef name ) {
    ...
    void XmlReporter::benchmarkFailed(const StringRef error) {

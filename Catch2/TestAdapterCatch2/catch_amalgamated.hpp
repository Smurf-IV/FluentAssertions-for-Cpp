...
namespace Catch {
    ...
    public: // construction
        ...
        constexpr StringRef( char const* rawChars, const size_type size ) noexcept
        :   m_start( rawChars ),
            m_size( size )
        {}
    ...
    public: // operators
        ...
        constexpr auto operator[] (const size_type index ) const noexcept -> char {
            assert(index < m_size);
    ...
    public: // named queries
        ...
        constexpr StringRef substr(const size_type start, const size_type length) const noexcept {
            if (start < m_size) {
    ...
    constexpr auto operator ""_sr( char const* rawChars, const std::size_t size ) noexcept -> StringRef {
        return StringRef( rawChars, size );
    }
} // namespace Catch

constexpr auto operator ""_catch_sr( char const* rawChars, const std::size_t size ) noexcept -> Catch::StringRef {
...
namespace Catch {
    ...
    inline bool isFalseTest(const int flags ) { return ( flags & ResultDisposition::FalseTest ) != 0; }
...
namespace Catch {
    namespace Benchmark {
        ...
        public:
            ...
            Chronometer(Detail::ChronometerConcept& meter, const int repeats_)
                : impl(&meter)
                , repeats(repeats_) {}
    ...
    namespace Benchmark {
        namespace Detail {
            ...
            public:
                ...
                void operator()(const Chronometer meter) const { f->call(meter); }
    ...
    namespace Benchmark {
        namespace Detail {
            ...
            sample jackknife(Estimator&& estimator,
                             const std::vector<double>::iterator first,
            ...
            template <typename Estimator>
            Estimate<double> bootstrap(const double confidence_level,
                ...
                const double jack_mean = mean(jack.begin(), jack.end());
                double sum_squares = 0, sum_cubes = 0;
                for (const double x : jack) {
                    const auto difference = jack_mean - x;
                    const auto square = difference * difference;
                    const auto cube = square * difference;
                    sum_squares += square; sum_cubes += cube;
                }

                double accel = sum_cubes / (6 * std::pow(sum_squares, 1.5));
                long n = static_cast<long>(resample.size());
                const double prob_n = std::count_if(resample.begin(), resample.end(), [point](const double x) { return x < point; }) / static_cast<double>(n);
                ...
                const double z1 = normal_quantile((1. - confidence_level) / 2.);

                auto cumn = [n](const double x ) -> long {
                    return std::lround( normal_cdf( x ) * static_cast<double>(n) );
                };
                auto a = [bias, accel](const double b) { return bias + b / (1. - accel * b); };
                const double b1 = bias + z1;
                const double b2 = bias - z1;
                const double a1 = a(b1);
                const double a2 = a(b2);
                const auto lo = static_cast<size_t>((std::max)(cumn(a1), 0l));
                const auto hi = static_cast<size_t>((std::min)(cumn(a2), n - 1));
...
namespace Catch {
    namespace Benchmark {
        namespace Detail {
            template <typename Clock>
            std::vector<double> resolution(const int k) {
            ...
            EnvironmentEstimate<FloatDuration<Clock>> estimate_clock_cost(FloatDuration<Clock> resolution) {
                ...
                const int nsamples = static_cast<int>(std::ceil(time_limit / r.elapsed));
...
namespace Catch {
    ...
    public:
        ...
        template<typename E>
        Detail::EnumInfo const& registerEnum(const StringRef enumName, const StringRef allEnums, std::initializer_list<E> values ) {
...
namespace Catch {
    ...
    namespace Detail {
        ...
        inline std::size_t catch_strnlen(const char *str, const std::size_t n) {
            const auto ret = std::char_traits<char>::find(str, n, '\0');
            if (ret != nullptr) {
...
# endif
    ...
    struct StringMaker<signed char[SZ]> {
        static std::string convert(signed char const* str) {
            const auto reinterpreted = reinterpret_cast<char const*>(str);
            return Detail::convertIntoString(
    ...
    struct StringMaker<bool> {
        static std::string convert(const bool b) {
...
namespace Catch {
    ...
    struct SourceLineInfo {
        ...
        constexpr SourceLineInfo( char const* _file, const std::size_t _line ) noexcept:
            file( _file ),
            line( _line )
        {}
...
namespace Catch {
    ...
    class LazyExpression {
        ...
        bool m_isNegated;
    public:
        LazyExpression(const bool isNegated ):
...
namespace Catch {
    ...
    struct MessageBuilder : MessageStream {
        MessageBuilder(const StringRef macroName,
                       SourceLineInfo const& lineInfo,
                       const ResultWas::OfType type ):
    ...
    public:
        ...
        template<typename T>
        void captureValues(const size_t index, T const& value ) {
            captureValue( index, Catch::Detail::stringify( value ) );
        }

        template<typename T, typename... Ts>
        void captureValues(const size_t index, T const& value, Ts const&... values ) {
            captureValue( index, Catch::Detail::stringify(value) );
...
namespace Catch {
    namespace Clara {
        ...
        namespace Detail {
            ...
            class ResultBase {
            protected:
                ResultBase(const ResultType type ): m_type( type ) {}
            ...
            protected:
                ResultValueBase(const ResultType type ): ResultBase( type ) {}
        ...
        public:
            ...
            std::string const& name() const { return *m_name; }
            Detail::ParserResult set(std::string const& newName) const;
        };
...
namespace Catch {
    ...
    struct TagAlias {
        TagAlias(std::string const& _tag, const SourceLineInfo _lineInfo):
            tag(_tag),
            lineInfo(_lineInfo)
        {}
...
namespace Catch {
    ...
    public:
        ...
        ITransientExpression(const bool isBinaryExpression, const bool result )
    ...
    public:
        BinaryExpr(const bool comparisonResult, LhsT lhs, const StringRef op, RhsT rhs )
...
struct NameAndTags {
    constexpr NameAndTags(const StringRef name_ = StringRef(),
                          const StringRef tags_ = StringRef() ) noexcept:
...
namespace Catch {
     ...
     */
    struct Tag {
        constexpr Tag(const StringRef original_):
            original(original_)
        {}
        StringRef original;
...
namespace Catch {
    ...
    class ExceptionTranslatorRegistrar {
        ...
        public:
            ...
            std::string translate(const ExceptionTranslators::const_iterator it, const ExceptionTranslators::const_iterator itEnd ) const override {
#if !defined(CATCH_CONFIG_DISABLE_EXCEPTIONS)
                try {
                    if( it == itEnd )
                        std::rethrow_exception(std::current_exception());
...
namespace Detail {
    ...
    template<typename L>
    auto generate(const StringRef generatorName, SourceLineInfo const& lineInfo, L const& generatorExpression ) -> typename decltype(generatorExpression())::type {
...
namespace Generators {
    ...
    class TakeGenerator final : public IGenerator<T> {
        ...
        size_t m_target;
    public:
        TakeGenerator(const size_t target, GeneratorWrapper<T>&& generator):
            m_generator(CATCH_MOVE(generator)),
            m_target(target)
        {
    ...
    public:
            ...
            m_predicate(CATCH_FORWARD(pred))
        {
            if (!m_predicate(m_generator.get())) {
                ...
                const auto has_initial_value = next();
    ...
    public:
        RepeatGenerator(const size_t repeats, GeneratorWrapper<T>&& generator):
            m_generator(CATCH_MOVE(generator)),
    ...
    public:
        ChunkGenerator(const size_t size, GeneratorWrapper<T> generator) :
...
namespace Catch {
    ...
    private:
        ...
        static constexpr std::uint64_t s_inc = (0x13ed0cc53f939476ULL << 1ULL) | 1ULL;
    };
...
public:
    RandomFloatingGenerator( Float a, Float b, const std::uint32_t seed ):
        m_rng(seed),
        m_dist(a, b) {
...
public:
    RandomIntegerGenerator( Integer a, Integer b, const std::uint32_t seed ):
        m_rng(seed),
        m_dist(a, b) {
        static_cast<void>(next());
    }
...
namespace Catch {
    ...
    struct TestRunInfo {
        constexpr TestRunInfo(const StringRef _name) : name(_name) {}
        StringRef name;
    };
...
namespace TestCaseTracking {
    ...
    struct NameAndLocationRef {
        ...
        constexpr NameAndLocationRef(const StringRef name_,
                                     const SourceLineInfo location_ ):
...
namespace Catch {
    ...
    public:
        constexpr pluralise(const std::uint64_t count, const StringRef label):
            m_count(count),
            m_label(label)
        {}
...
namespace Catch {
    ...
    private:
        ...
        inline void addCharToPattern(const char c) {
            m_substring += c;
            m_patternName += c;
            m_realPatternPos++;
        }
...
namespace Catch {
    namespace TextFlow {
        ...
        public:
            ...
            Column& width(const size_t newWidth ) {
                assert( newWidth > 0 );
                m_width = newWidth;
                return *this;
            }
            Column& indent(const size_t newIndent ) {
                m_indent = newIndent;
                return *this;
            }
            Column& initialIndent(const size_t newIndent ) {
...
namespace Catch {
    ...
    public:
        ...
        template <typename T,
                  ...
                  typename = typename std::enable_if_t<
                      !std::is_convertible_v<T, StringRef>>>
        XmlWriter& writeAttribute(const StringRef name, T const& attribute ) {
            ReusableStringStream rss;
            rss << attribute;
...
namespace Matchers {
    ...
    namespace Detail {
        ...
        std::string describe_multi_matcher(const StringRef combine, std::array<void const*, sizeof...(MatcherTs)> const& matchers, std::index_sequence<Idx...>) {
...
namespace Catch {
    namespace Matchers {
        ...
        public:
            explicit HasSizeMatcher(const std::size_t target_size):
...
namespace Catch {
    ...
    struct lineOfChars {
        char c;
        constexpr lineOfChars(const char c_ ): c( c_ ) {}
...
namespace Catch {
    ...
    class ListenerRegistrar {
        ...
        public:
            TypedListenerFactory(const StringRef listenerName ):
